...
const int trigP = 12;  //D4 Or GPIO-2 of nodemcu
const int echoP = 14;  //D3 Or GPIO-0 of nodemcu
const int trigP2 = 5;  //D4 Or GPIO-2 of nodemcu
const int echoP2 = 4;  //D3 Or GPIO-0 of nodemcu
long duration;
int distance;
long duration2;
int distance2;
      ...
      for (int i = 0; i < GPIOkeys.length(); i++) {
        ...
        random_unneeded_string = httpGETRequest(SQL_req.c_str());
         Serial.print("GPIO: ");
          ...
          else if (echoP2==atoi(GPIOkeys[i]))
          {VALUE = distance2;
